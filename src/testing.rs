...
impl BlokliTestStateBuilder {
    ...
    /// Sets initial wxHOPR (token) balances for the given addresses.
    ///
    /// Each entry is `(Address, HoprBalance)`. Overwrites any existing token
    /// balance at that address.
    #[must_use]
    pub fn with_token_balances(mut self, balances: impl IntoIterator<Item = (Address, HoprBalance)>) -> Self {
    ...
    /// Sets initial xDai (native) balances for the given addresses.
    ///
    /// Each entry is `(Address, XDaiBalance)`. Overwrites any existing native
    /// balance at that address.
    #[must_use]
    pub fn with_native_balances(mut self, balances: impl IntoIterator<Item = (Address, XDaiBalance)>) -> Self {
        for (addr, balance) in balances {
            self.0.native_balances.insert(
                const_hex::encode(addr),
                blokli_client::api::types::NativeBalance {
