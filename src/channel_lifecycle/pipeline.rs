        ...
        + 'static,
{
    ...
    fn total_in_flight(&self) -> usize {
        self.open_in_flight.held_count()
            ...
            + self.close_in_flight.held_count()
    ...
    async fn read<T, E: std::fmt::Display>(
        ...
        match op.timeout(futures_time::time::Duration::from(budget)).await {
            ...
            Ok(Err(e)) => {
                warn!(%e, what, "channel-lifecycle: chain read failed");
                None
            }
            Err(_) => {
                warn!(what, ?budget, "channel-lifecycle: chain read timed out");
                None
            }
        }
    }
    ...
    async fn pipeline_inner(&self) -> crate::errors::Result<()> {
        ...
        let Some(all_channels) = self
            .read("stream_channels", deadline, async {
                ...
                Ok::<_, <N::ChainApi as ChainReadChannelOperations>::Error>(all_channels)
            })
            .await
        else {
        ...
        let min_onward_balance = ticket_price
            .as_ref()
            .zip(win_prob)
            .map(|(price, wp)| super::config::winning_ticket_face_value(*price, wp, super::config::ASSUMED_HOPS))
        ...
        // Only overwritten on success: a tick that could not read the economics
        // must not take away the last known-good values the event handler runs
        // on, or one failed read disables event-driven funding until a tick
        // succeeds.
        if funding.is_some() {
        ...
        let open_channels: Vec<&ChannelEntry> = all_channels
            ...
            .collect();
        let open_count = open_channels.len() + self.open_in_flight.held_count();
        debug!(
            open = open_count,
            in_flight = self.total_in_flight(),
            safe = safe_balance.map(|b| b.to_string()).unwrap_or_else(|| "unknown".into()),
        ...
        // stake the funding txs already staked in this same tick.  Zero when the
        ...
        if let (Some(funding), Some(safe_balance)) = (funding, safe_balance) {
            // Whether a channel drained into the close pass's balance-drained territory
            // (`balance <= close_when_drained_below`) should still be topped up here, or
            // left for the close pass to retire and reallocate:
            //
            // * balance above the drain threshold — not drain-close territory, always fundable.
            // * operator set a positive drain floor — an explicit "retire, don't refund"; never rescue a channel
            //   drained to it, regardless of the peer.
            // * default floor (zero): refund a fully-drained channel only when its peer is still worth keeping —
            //   connected, or with recent ticket activity — so a proven relay is kept alive; a drained channel to a
            //   disconnected, inactive peer is left for the close pass to retire. Connectivity is resolved as in the
            //   close pass: an unresolvable peer counts as connected (shielded).
            let refundable = |ch: &ChannelEntry| -> bool {
                if ch.balance > self.cfg.closure.close_when_drained_below {
                    return true;
                }
                if !self.cfg.closure.close_when_drained_below.is_zero() {
                    return false;
                }
                let connected = addr_to_key
                    .get(&ch.destination)
                    .map(|pk| self.node.network_view().is_connected(&PeerId::from(pk)))
                    .unwrap_or(true);
                let active = self.peer_ticket_activity.get(&ch.destination).is_some_and(|v| *v > 0);
                connected || active
            };
            ...
            debug!(
                ...
                "channel-lifecycle: fund pass skipped: chain inputs unavailable"
            );
        }
        ...
        let connected = match peer_addr_map.as_ref() {
            Some(_) => self.node.network_view().connected_peers(),
        ...
        let open_candidates: Vec<OpenCandidate> = connected
            .into_iter()
            .filter_map(|peer_id| {
                let &(offchain_key, chain_addr) = peer_addr_map.as_ref()?.get(&peer_id)?;
                if chain_addr == me {
                    return None;
                }
    ...
    async fn peer_addr_map(
        ...
        let map = self
            .read("stream_accounts", deadline, async {
                ...
                Ok::<_, <N::ChainApi as ChainReadAccountOperations>::Error>(map)
            })
            .await?;
        ...
        let safe_map: HashMap<Address, Address> = {
            let accounts = self
                .read("stream_accounts (stake view)", deadline, async {
                    ...
                    Ok::<_, <N::ChainApi as ChainReadAccountOperations>::Error>(m)
                })
                .await;
            match accounts {
                Some(m) => m,
                None => return StakeView::empty(),
            }
        };
...
mod tests {
    ...
    /// enough that nothing expires mid-test.
    const TEST_LEASE: Duration = Duration::from_secs(3600);
    ...
    impl hopr_api::network::NetworkView for StubNetworkView {
        ...
        fn is_connected(&self, peer: &PeerId) -> bool {
            // Unconfigured (no peer marked connected) means "connectivity is not
            // under test": report every peer connected, so the connectivity close
            // trigger stays inert unless a test opts in via `connect`. Once any peer
            // is marked, fall back to exact membership.
            self.connected.is_empty() || self.connected.contains_key(peer)
        }
    ...
    fn new_instance_should_have_empty_state_after_old_dropped() {
        ...
        inner1.open_in_flight.acquire(*DAVE, TEST_LEASE);
    ...
    async fn inflight_events_arrive_at_new_instance_after_restart() -> anyhow::Result<()> {
        ...
        // ── Simulate old instance: accumulate in-flight state, then drop ─────
        {
            ...
            old.fund_in_flight.acquire(*ch_fund.get_id(), TEST_LEASE);
            old.open_in_flight.acquire(ch_open.destination, TEST_LEASE);
    ...
    /// Build an inner over a `ChainNode` with a configurable network view — for tests
    /// that drive connectivity through the `ChainNode`-based stub.
    fn fresh_inner_with_chain_graph_and_network<C>(
        ...
        fresh_inner_over(
            ...
            Arc::new(ChainNode::with_graph_and_network(connector, graph, network)),
        )
    }
    ...
    async fn fetch_forwarding_view_counts_outgoing_channels() -> anyhow::Result<()> {
        ...
        let min_onward = super::super::config::winning_ticket_face_value(
            HoprBalance::new_base(1),
            1.0,
            super::super::config::ASSUMED_HOPS,
        );
        let view = inner
